...
def read_workbook(file_obj) -> Dict[str, pd.DataFrame]:
    ...
    cleaned = {sheet: clean_dataframe(df) for sheet, df in workbook.items()}
...
def prepare_data(data: Dict[str, pd.DataFrame]) -> Dict[str, pd.DataFrame]:
    ...
    code_columns = {
        ...
        "Nilai_CPMK": ["NIM", "Kode MK", "Kode CPMK"],
    }
...
def get_target_value(target_df: pd.DataFrame, parameter: str, default: float) -> float:
    if target_df.empty:
        return default
    mask = target_df["Parameter"].astype(str).str.strip().str.lower() == parameter.lower()
...
def add_period_columns(df: pd.DataFrame, periode: str, tahun_akademik: str, semester: str) -> pd.DataFrame:
    ...
    if "Semester" in output.columns:
        output = output.rename(columns={"Semester": "Semester MK"})
    for column, value in [
        ("Semester", semester),
...
def calculate_rekap_cpmk(
    ...
    grouped = (
        nilai_valid.groupby("Kode CPMK", as_index=False)
        .agg(
            **{
    ...
    rekap = mapping_unique.merge(grouped, on="Kode CPMK", how="left")
    fill_zero_columns = [
        "Jumlah Mahasiswa",
    ...
    return rekap.sort_values(["Kode CPL", "Kode IK", "Kode CPMK"]).reset_index(drop=True)
...
def calculate_all(
    ...
    batas = (
        ...
        else get_target_value(prepared["Target"], "Batas Nilai Minimum", 70)
    )
    target = (
        float(target_cpl)
        if target_cpl is not None
        else get_target_value(prepared["Target"], "Target Ketercapaian CPL", 70)
    )
...
def make_sample_data() -> Dict[str, pd.DataFrame]:
    ...
    mapping = pd.DataFrame(
        [
            [1, "TE101", "Rangkaian Listrik", "CPMK1", "Mahasiswa mampu menghitung parameter rangkaian DC.", "IK1.1", "CPL1", 2],
            [1, "TE102", "Fisika Terapan", "CPMK2", "Mahasiswa mampu menerapkan konsep fisika pada sistem elektro.", "IK1.2", "CPL1", 1],
            [2, "TE201", "Dasar Elektronika", "CPMK3", "Mahasiswa mampu mengidentifikasi gangguan komponen elektronika.", "IK2.1", "CPL2", 1],
            [2, "TE202", "Troubleshooting Elektronika", "CPMK4", "Mahasiswa mampu memilih solusi troubleshooting.", "IK2.2", "CPL2", 2],
            [3, "TE301", "Pengukuran Elektronika", "CPMK5", "Mahasiswa mampu menggunakan osiloskop dan multimeter.", "IK3.1", "CPL3", 1],
            [3, "TE302", "Simulasi Rangkaian", "CPMK6", "Mahasiswa mampu menggunakan software simulasi rangkaian.", "IK3.2", "CPL3", 1],
        ],
...
def render_single_mode() -> None:
    ...
    default_batas = get_target_value(prepared["Target"], "Batas Nilai Minimum", 70)
    default_target = get_target_value(prepared["Target"], "Target Ketercapaian CPL", 70)
    ...
    result = calculate_all(raw_data, batas_nilai, target_cpl)
